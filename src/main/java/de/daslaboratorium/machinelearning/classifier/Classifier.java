...
public abstract class Classifier<T, K> implements IFeatureProbability<T, K>, java.io.Serializable {
     ...
     */
    private static final int INITIAL_CATEGORY_DICTIONARY_CAPACITY = 5;
     ...
     */
    private static final int INITIAL_FEATURE_DICTIONARY_CAPACITY = 25000;
    ...
    public BigDecimal featureProbability(T feature, K category) {
        final BigDecimal totalFeatureCount = new BigDecimal(this.getFeatureCount(feature));

        if (totalFeatureCount.compareTo(BigDecimal.ZERO) == 0) {
            return BigDecimal.ZERO;
        } else {
            return new BigDecimal(this.getFeatureCount(feature, category)).divide(new BigDecimal(this.getFeatureCount(feature)), 2 , RoundingMode.HALF_UP );
        }
    }
     ...
     */
    public BigDecimal featureWeighedAverage(T feature, K category) {
     ...
     * Object, IFeatureProbability, float, float)
     */
    public BigDecimal featureWeighedAverage(T feature, K category, IFeatureProbability<T, K> calculator) {
     ...
     * Object, IFeatureProbability, float, float)
     */
    public BigDecimal featureWeighedAverage(T feature, K category, IFeatureProbability<T, K> calculator, float weight) {
    ...
    public BigDecimal featureWeighedAverage(T feature, K category, IFeatureProbability<T, K> calculator, float weight,
         ...
         * category.
         */
        final BigDecimal basicProbability = (calculator == null) ? this.featureProbability(feature, category)
                : calculator.featureProbability(feature, category);

        Integer totals = this.totalFeatureCount.get(feature);
        if (totals == null) totals = 0;
        return (basicProbability.multiply(new BigDecimal(totals)).add(BigDecimal.valueOf(weight * assumedProbability))).divide(BigDecimal.valueOf(weight + totals), 2 , RoundingMode.HALF_UP);
    }
